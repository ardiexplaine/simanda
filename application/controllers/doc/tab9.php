...
class Tab9 extends CI_Controller {
		...
		if($query->num_rows() > 0){
			...
			if($ctg == 'WF01') {
				$html .= '<div class="col-lg-12">';
					...
					$html .= '<table id="divtbltab9" class="table table-striped table-condensed" data-provides="rowlink">
						<thead>
							<tr>
								<th>NO</th>
								<th>KODE URUSAN</th>
								<th>KEGIATAN PINJAMAN DAERAH YANG SESUAI DALAM RKPD PEMERINTAH DAERAH </th>
								<th>JUMLAH</th>
								<th>#</th>
							</tr>
						</thead>
					...
					$html .='<tr class="rowlink">
								...
								<td><button '.$disabled.' onclick="tab9delitem('."'".$row->item_id."'".');"><i class="splashy-document_letter_remove"></i> Hapus</button></td>
                            </tr>';     
                        $no++;
                        $total += $row->amount;
                    }
    ...
    function savedata() {
		...
		$this->form_validation->set_rules('tab9_title','Belanja Pegawai','required|xss_clean');
        $this->form_validation->set_rules('tab9_amount','Belanja Tambahan Penghasilan PNS','required|xss_clean');


        $lk_kegiatan_honor = array(
            "wfnum"  => $this->input->post("txtWfnum"),
			"ktg_id" => $this->ktgid,
			"urusan_id"  => $this->input->post("tab9_urusan_id"),
            "title"  => $this->input->post("tab9_title"),
            "amount" => str_replace(",","", $this->input->post("tab9_amount"))
        );
