...
class Tab7 extends CI_Controller {
		...
		if($query->num_rows() > 0){
			...
			if($ctg == 'WF01') {
				$html .= '<div class="col-lg-12">';
					...
					$html .= '<table id="divtbltab7" class="table table-striped" data-provides="rowlink">
						<thead>
							<tr>
								<th>No</th>
					...
					$html .='<tr class="rowlink">
								...
								<td><button '.$disabled.' onclick="tab7delitem('."'".$row->item_id."'".');"><i class="splashy-document_letter_remove"></i> Hapus</button></td>
                            </tr>';     
                        $no++;
                        $total += $row->amount;
                    }
    ...
    function savedata() {
		...
		$this->form_validation->set_rules('tab7_title','Belanja Pegawai','required|xss_clean');
        $this->form_validation->set_rules('tab7_amount','Belanja Tambahan Penghasilan PNS','required|xss_clean');


        $lk_kegiatan_honor = array(
            "wfnum"  => $this->input->post("txtWfnum"),
            "ktg_id" => $this->ktgid,
            "title"  => $this->input->post("tab7_title"),
            "amount" => str_replace(",","", $this->input->post("tab7_amount"))
        );
