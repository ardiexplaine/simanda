...
class Tab4 extends CI_Controller {
		...
		if($query->num_rows() > 0){
			...
			if($ctg == 'WF01') {
				$html .= '<div class="col-lg-12">';
					...
					$rata2 = $total/3;
					$html .='<tr class="rowlink" data-toggle="modal" data-backdrop="static" href="#templateRataDscr" data-placement="bottom" data-container="body" onclick="tab4_modalDscr('.$row->wfnum.','.$row->dscr_id.','.$row->ktg_id.','.$row->list_id.','.$row->ta_1.','.$row->ta_2.','.$row->ta_3.');">
								<td>'.$no.'</td>
								<td>'.$row->list_name.'</td>
    ...
    function savedata() {
		...
		$this->form_validation->set_rules('tab4_wfnum','wfnum kosong','required|xss_clean');
		$this->form_validation->set_rules('tab4_dscr_id','dscr kosong','required|xss_clean');
		$this->form_validation->set_rules('tab4_ktg_id','kategori id kosong','required|xss_clean');


		$total = $this->input->post("tab4_ta_1")+$this->input->post("tab4_ta_2")+$this->input->post("tab4_ta_3");
        $lk_kegiatan_honor = array(
            "ta_1"  => str_replace(",","", $this->input->post("tab4_ta_1")),
            "ta_2"  => str_replace(",","", $this->input->post("tab4_ta_2")),
            "ta_3"  => str_replace(",","", $this->input->post("tab4_ta_3")),
            "total" => str_replace(",","", $total)
        );
		//echo '<pre/>'; print_r($_POST); exit;
		
		if($this->form_validation->run() == TRUE){
            $this->db->update('lk_dscr', $lk_kegiatan_honor, array("dscr_id"=> $this->input->post("tab4_dscr_id")));
            $status = 0;
