...
class Tab6 extends CI_Controller {
			...
			if($ctg == 'WF01') {
				$html .= '<div class="col-lg-9">';
					...
					$html .='<tr class="rowlink">
								...
								<td><button '.$disabled.' onclick="tab6delitem('."'".$row->item_id."'".');"><i class="splashy-document_letter_remove"></i> Hapus</button></td>
                            </tr>';     
                        $no++;
                        $total += $row->amount;
                    }
    ...
    function savedata() {
		...
		$this->form_validation->set_rules('tab6_title','Belanja Pegawai','required|xss_clean');
        $this->form_validation->set_rules('tab6_amount','Belanja Tambahan Penghasilan PNS','required|xss_clean');


        $lk_kegiatan_honor = array(
            "wfnum"  => $this->input->post("txtWfnum"),
            "ktg_id" => $this->ktgid,
            "title"  => $this->input->post("tab6_title"),
            "amount" => str_replace(",","", $this->input->post("tab6_amount"))
        );
